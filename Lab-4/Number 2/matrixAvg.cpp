...
double calculatePositiveAvg(int rows, int cols, int matrix[][max_size]){
    double sum = 0;
    int count = 0;
    double result = 0;
...
void productElements(int rows, int cols, int matrix[max_size][max_size]){
    for (int i = 0; i < rows; i++) {
        int product = 1;
...
void matrixAvg(){
    ...
    double avg1 = calculatePositiveAvg(M1, N1, a);
    double avg2 = calculatePositiveAvg(M2, N2, b);

    if (avg1 < avg2) {
        std::cout << "Матрица A. Произведение ненулевых элементов по строкам:\n";
        productElements(M1, N1, a);
    } else {
        std::cout << "Матрица B. Произведение ненулевых элементов по строкам:\n";
        productElements(M2, N2, b);
    }
}